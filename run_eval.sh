...
SERVER_SCRIPT="server/server_eval.py"
...
nohup python ${SERVER_SCRIPT} --config ${SERVER_CONFIG} > ${SERVER_LOG} 2>&1 &
SERVER_PID=$!
...
nohup python client/client_eval.py --dataset_path ${DATASET_A} --server_address ${SERVER_ADDRESS} > ${CLIENT1_LOG} 2>&1 &
CLIENT1_PID=$!
echo "Client 1 started with PID: ${CLIENT1_PID}"

# Client 2 with dataset B.
echo "Starting Client 2 with ${DATASET_B}..."
nohup python client/client_eval.py --dataset_path ${DATASET_B} --server_address ${SERVER_ADDRESS} > ${CLIENT2_LOG} 2>&1 &
CLIENT2_PID=$!
