...
from flwr.common import parameters_to_ndarrays  # added helper to convert Parameters to list[np.ndarray]
...
# Custom strategy with intermediate evaluation by subclassing FedAvg
class FedAvgWithIntermediateEval(fl.server.strategy.FedAvg):
    def __init__(self, global_model, test_loader, **kwargs):
        super().__init__(**kwargs)
        self.global_model = global_model  # Global model used for evaluation
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        if aggregated_parameters is not None:
            # Parameters to list of array
            ndarrays = parameters_to_ndarrays(aggregated_parameters)
            state_dict = self.global_model.state_dict()
            # new state dict - mapping each key with its array
            new_state_dict = {
